...
export class AdminController{
  ...
  // --- Usuários (todos os perfis do tenant) ---------------------------------
  @Roles('ADMIN','SUPPORT','FINANCE') @Get('users') users(@CurrentUser()u:AuthenticatedUser,@Query()q:ListUsersQueryDto){return this.service.listUsers(u,q)}
  @Roles('ADMIN') @Idempotent() @Post('users') createUser(@CurrentUser()u:AuthenticatedUser,@Body()d:CreateUserDto){return this.service.createUser(u,d)}
  // Remoção lógica: bloqueia o acesso e revoga as sessões, preservando histórico.
  @Roles('ADMIN') @Delete('users/:id') blockUser(@CurrentUser()u:AuthenticatedUser,@Param('id')id:string,@Body()d:BlockUserDto){return this.service.blockUser(u,id,d)}
  @Roles('ADMIN') @Post('users/:id/restore') restoreUser(@CurrentUser()u:AuthenticatedUser,@Param('id')id:string){return this.service.unblockUser(u,id)}

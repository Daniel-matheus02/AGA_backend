...
export class CreditService {
  ...
  async create(user:AuthenticatedUser,dto:CreateCreditRequestDto){
    ...
    const pending=await this.prisma.creditRequest.findFirst({where:{userId:user.sub,status:{in:['PENDING','UNDER_REVIEW']}}});
    ...
    const daily=calculateDailyAmount(amount,dto.dailyInstallments);
    return this.prisma.$transaction(async tx=>{
      const request=await tx.creditRequest.create({data:{tenantId:user.tenantId,userId:user.sub,amountCents:amount,dailyInstallments:dto.dailyInstallments,dailyAmountCents:daily}});
      await this.events.append({tenantId:user.tenantId,type:'credit.request.created',aggregateType:'CreditRequest',aggregateId:request.id,payload:{requestId:request.id,userId:user.sub,amountCents:amount.toString(),dailyInstallments:dto.dailyInstallments},audience:[`user:${user.sub}`,'role:ADMIN','role:FINANCE']},tx);
      return request;
    });
  }
